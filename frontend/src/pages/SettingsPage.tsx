...
import type { LibraryRoot, LibraryRootIn, PathValidationResult } from "../types/video";
      ...
      <section className="settings-section">
        <div className="settings-section-header">
          <div>
            ...
            <p className="settings-section-desc">
              Configure folders that the scanner indexes for video files.
              <br />
              <strong>Important:</strong> Use container paths (e.g. <code>/media/library/gopro</code>), not NAS host
              paths. Mount host dirs in <code>docker-compose.yml</code> first.
            </p>
          </div>
        ...
        {scanMsg && (
          <div className="settings-notice">
            {scanMsg}{" "}
            <button className="btn-link" onClick={() => setScanMsg(null)}>
              ✕
            </button>
          </div>
        )}
        {error && (
          <div className="settings-error">
            {error}{" "}
            <button className="btn-link" onClick={() => setError(null)}>
              ✕
            </button>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="settings-empty">
            No media sources configured. Click <strong>+ Add Source</strong> to add one.
            <br />
            <em>
              If you have <code>VIDEO_LIBRARY_PATH</code> set, a Default source will be created automatically on
              the next scan.
            </em>
          </div>
        ) : (
                ...
                {sources.map((s) => (
                  <tr key={s.id} className={s.enabled ? "" : "row-disabled"}>
                    ...
                    <td>
                      ...
                      {s.last_error && (
                        <span className="error-hint" title={s.last_error}>
                          {" "}⚠
                        </span>
                      )}
                    </td>
        ...
        <div className="modal-overlay" onClick={closeModal}>
          <div className="modal-box" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <label className="form-label">
                ...
                <div className="input-row">
                  <input
                    ...
                    placeholder="/media/library/gopro"
                  />
                  <button
                ...
                <span className="form-hint">
                  Enter the path as seen <em>inside the Docker container</em> (e.g.{" "}
                  <code>/media/library/gopro</code>). On Synology, mount host paths in{" "}
              ...
              {validation && (
                <div
                  className={`validation-result ${validation.valid ? "validation-ok" : "validation-fail"}`}
                >
                  {validation.valid ? "✓ " : "✗ "}
                  {validation.message}
                </div>
